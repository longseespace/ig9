...
class Message extends CActiveRecord
{
	...
	const DELETED_BY_BOTH = 'both';
	const DELETED_BY_SENDER_EXPLICIT = 'sender-explicit';
	const DELETED_BY_RECEIVER_EXPLICIT = 'receiver-explicit';
	...
	public static function getAdapterForInbox($userId) {
		...
		$c->addCondition('t.deleted_by NOT IN (:deleted_by_receiver, :deleted_by_receiver_explicit, :deleted_by_both) OR t.deleted_by IS NULL');
		$c->order = 't.created_at DESC';
		$c->params = array(
			'receiverId' => $userId,
			'deleted_by_receiver' => Message::DELETED_BY_RECEIVER,
			'deleted_by_receiver_explicit' => Message::DELETED_BY_RECEIVER_EXPLICIT,
			'deleted_by_both' => Message::DELETED_BY_BOTH,
		);
	...
	public static function getAdapterForSent($userId) {
		...
		$c->addCondition('t.receiver_id != 0');
		$c->addCondition('t.deleted_by NOT IN (:deleted_by_sender, :deleted_by_sender_explicit, :deleted_by_both) OR t.deleted_by IS NULL');
		$c->order = 't.created_at DESC';
		$c->params = array(
			'senderId' => $userId,
			'deleted_by_sender' => Message::DELETED_BY_SENDER,
			'deleted_by_sender_explicit' => Message::DELETED_BY_SENDER_EXPLICIT,
			'deleted_by_both' => Message::DELETED_BY_BOTH,
		);
		$messagesProvider = new CActiveDataProvider('Message', array('criteria' => $c));
		return $messagesProvider;
	...
	public static function getAdapterForTrash($userId) {
		$c = new CDbCriteria();
		$c->addCondition('t.receiver_id = :receiverId');
		$c->addCondition('t.deleted_by = :deleted_by_receiver');
		$c->order = 't.created_at DESC';
		$c->params = array(
			'receiverId' => $userId,
			'deleted_by_receiver' => Message::DELETED_BY_RECEIVER,
