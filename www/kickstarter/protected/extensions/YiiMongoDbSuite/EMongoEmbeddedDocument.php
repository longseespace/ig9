...
abstract class EMongoEmbeddedDocument extends CModel
{
	...
	{
		// @HACK, polyfill with sql id
		if($name == 'id' && !empty($this->_id)){
			return $this->_id;
		}else if($this->hasEmbeddedDocuments() && isset(self::$_embeddedConfig[get_class($this)][$name])) {
	...
	public function __set($name, $value)
	{
		...
		if($name == 'id'){
			$this->_id = $value;
		}elseif($this->hasEmbeddedDocuments() && isset(self::$_embeddedConfig[get_class($this)][$name])){
			if(is_array($value)) {
				// Late creation of embedded documents on first access
				if (is_null($this->_embedded->itemAt($name))) {
					$docClassName = self::$_embeddedConfig[get_class($this)][$name];
			...
			else if($value instanceof CActiveRecord)
				return $this->_embedded->add($name, $value);
		}
		else
