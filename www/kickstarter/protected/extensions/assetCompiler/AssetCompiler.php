...
class AssetCompiler extends CApplicationComponent
{
  ...
  public function registerAssetGroup($group, $baseUrl=null)
  {
    if (is_array($group))
    {
      foreach ($group as $g)
      {
        $this->registerAssetGroup($g, $baseUrl);
      }
    }
    else
    {
      if ($this->debugMode)
        $this->registerAssetGroupRaw($group, $baseUrl);
      else
        $this->registerAssetGroupCompiled($group, $baseUrl);
    }
  }


  public function registerAssetGroupRaw($group, $baseUrl=null)
  {
    ...
    if ($g['type'] == 'js')
    {
      ...
      {
        $this->registerFile($f, $g['type'], $baseUrl);
      }
    }
    elseif ($g['type'] == 'less')
    {
      $this->registerFile($g['file'], $g['type'], $baseUrl);
    }

  }

  public function registerAssetGroupCompiled($group, $baseUrl=null)
  {
    $this->checkGroupExists($group);
    ...
    $this->registerFile($g['output'], $type, $baseUrl);
  }

  public function registerFile($file, $type, $baseUrl = null)
  {
    $file = str_replace(Yii::app()->theme->baseUrl.'/assets/', '', $file);
    $baseUrl = $baseUrl===null ? $this->baseUrl : $baseUrl;

    if ($type == 'js')
    {
      Yii::app()->clientScript->registerScriptFile($baseUrl.'/'.$file, $this->jsRenderPosition);
    }
    elseif ($type == 'css')
    {
      Yii::app()->clientScript->registerCssFile($baseUrl.'/'.$file);
    }
    elseif ($type == 'less')
    {
      Yii::app()->clientScript->registerScriptFile($this->lessJsUrl);
      Yii::app()->clientScript->registerLinkTag('stylesheet/less','text/css',$baseUrl.'/'.$file);
    }
    else
    {
