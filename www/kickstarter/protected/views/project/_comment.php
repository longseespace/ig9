...
<div class="backer comment<?php if ($project->user_id == $data->user_id) echo ' creator' ?>">
  ...
  <div class="meta">
    ...
    <div class="actions">
      ...
      <a href="<?php echo $this->createUrl('projectComment/delete', array('id' => $data->id)); ?>" class="delete" title="<?php __('Delete'); ?>"><span class="icon-trash"></span></a>
      <?php endif; ?>
    </div>
    <div class="content">
    ...
    <?php if (!Yii::app()->user->isGuest && (Yii::app()->user->id == $data->user_id || Yii::app()->user->isAdmin())) : ?>
    <div class="editor">
      <form action="<?php echo $this->createUrl('projectComment/edit', array('id' => $data->id)); ?>" method="POST">
